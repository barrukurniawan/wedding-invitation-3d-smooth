...
const isMetaKey = (key) => key === 'meta' || key === 'os';
export const useKeyboard = (optionsFn) => {
    const keys = new Map();
    const pendingDown = new Set();
    const pendingUp = new Set();
    const metaModifiedKeys = new Set();
    const hasMetaDown = () => {
        return (pendingDown.has('meta') ||
            pendingDown.has('os') ||
    ...
    const handleKeyUp = (event) => {
        ...
        for (const fn of listeners.keyup)
            fn(e);
    };
    const handleBlur = () => {
     ...
     */
    const { task } = useTask(Symbol('useKeyboard'), () => {
