 ...
 * standard layout.
 */
export const parseGamepadSignature = (id) => {
    const chromeMatch = id.match(/Vendor:\s*([0-9a-f]{4}).*?Product:\s*([0-9a-f]{4})/i);
    if (chromeMatch)
        return `${chromeMatch[1].toLowerCase()}:${chromeMatch[2].toLowerCase()}`;
    const firefoxMatch = id.match(/^([0-9a-f]{4})-([0-9a-f]{4})/i);
    if (firefoxMatch)
        return `${firefoxMatch[1].toLowerCase()}:${firefoxMatch[2].toLowerCase()}`;
    return null;
};
/**
 ...
 */
export const resolveMapping = (pad, userMappings, includeBuiltins) => {
 ...
 */
export const readButton = (pad, source, defaultIndex) => {
    if (!source)
        return pad.buttons[defaultIndex];
    if ('button' in source)
        return pad.buttons[source.button];
 ...
 */
export const readStick = (pad, mapping, defaultXAxis, defaultYAxis) => {
