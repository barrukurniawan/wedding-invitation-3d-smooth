...
{#if $activeModal === 'weddingStage'}
  ...
  >
    <div class="relative max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-2xl border border-rose-200/20 bg-stone-950 text-stone-100 shadow-2xl">
      {#if showPayment}
        <button onclick={() => (showPayment = false)} class="absolute left-3 top-3 z-10 rounded-full bg-black/35 px-2 py-1 text-sm text-white/80 backdrop-blur hover:text-white">←</button>
      ...
      {:else}
        ...
        <div class="px-5 pt-5 pb-3 text-center">
          <p class="text-base font-semibold text-rose-400">Halo, {$guestName}.</p>
          <h3 class="mt-1 font-serif text-lg text-pink-300">{countdown} menuju hari pernikahan kami</h3>
        </div>
